"""
================================================================================
SCRIPT 04 — THERMODYNAMIC & GEOMETRIC MODELING WITH PHOEBE 2
================================================================================
...
Methodology Step 4.4:
    Loads the cleaned, phase-folded TESS light curve into a PHOEBE 2 bundle.
    Assigns initial system parameters (including T1 from spectral type tables).
    Constructs the 3D stellar mesh, runs the forward model (synthetic LC), and
    defines the free parameters for the inverse problem solver in Script 05.

Dependencies:
    pip install phoebe lightkurve numpy matplotlib astropy

Inputs:
    neglected_targets.csv          — target list from Script 01
    lc_<starname>_folded.fits      — phase-folded light curves from Script 02
    ephem_<starname>.txt           — refined ephemeris from Script 03

Outputs:
    bundle_<starname>.phoebe       — saved PHOEBE 2 bundle (pre-solver state)
    plots/phoebe_init_<starname>.png — initial model vs. observations
================================================================================
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# SECTION 1: SPECTRAL TYPE → PRIMARY TEMPERATURE (Carroll & Ostlie 2017)
# ─────────────────────────────────────────────────────────────────────────────
SPECTRAL_TYPE_TEFF = {
...
for target in targets:
    ...
    # FIX: Bin the data to ~150 points to prevent PHOEBE from freezing the computer
    ...
    b.set_value("ecc@binary",    0.0)          # circular orbit assumed
    ...
    # Castelli & Kurucz log(g) out-of-bounds crashes.
    ...
    if is_contact:
        # FIX 2: To safely set geometry for a contact binary, we MUST flip the 
        # constraint so fillout_factor is free, and the radius adjusts dynamically.
        try:
            b.flip_constraint('fillout_factor@contact_envelope', solve_for='requiv@primary')
            b.set_value("fillout_factor@contact_envelope", init.get("fillout_init", 0.15))
            b.set_value("q@binary", init["q_init"]) # Safest to set q AFTER flipping
            print(f"  Contact morphology: fillout_factor={init.get('fillout_init', 0.15):.2f}, q={init['q_init']}")
        except Exception as e:
            print(f"  Contact geometry note: {e}")
    else:
    ...
    # limb darkening. We must set it to manual for BOTH stars explicitly.
